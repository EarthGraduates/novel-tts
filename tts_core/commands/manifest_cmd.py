"""tts manifest — Open chapter manifest for editing."""
...
def run(args):
    ...
    if args:
        ...
        if len(novels) > 1:
            ...
            for n in novels:
                print(f"  tts manifest {n.replace('_novel.json', '')}")
            sys.exit(1)
    ...
    print("   S  — 需拆分（用 tts view <ID> 标记拆点）")
    print("   X  — 标记为 front_matter，不朗读")
    print()
    print("   编辑完后运行 tts apply 应用修改")
